...
//	    conf.setInt(Setup.D_PARAM_SIZE, 15);
	    ...
	    while(k < args.length) {
	    	if(args[k].equals("-k")||args[k].equals("--cluster")) {
	    		...
	    		if(flag_p == 1) System.exit(usage()); else flag_p++;
	    		...
	    		if(flag_r == 1) System.exit(usage()); else flag_r++;
	...
	public int run(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
	    ...
	    if(!fs.exists(new Path(conf.get(Setup.JOB_PATH)+"/data.norm"))) {
		    ...
		    if(res==1) return res;
			Resources.appendFile(conf, "Normalization: "+Resources.timeParse(LocalDateTime.now().toEpochSecond(ZoneOffset.ofHours(-3))-timenow), "Time.meta", conf.get(Setup.JOB_PATH));
	    }
//	    if(!fs.exists(new Path(conf.get(Setup.JOB_PATH)+"/data.norm"))) return usage();
	    int kmeans_index = 1;
		Path kmeans_datapath = new Path(conf.get(Setup.JOB_PATH)+"/data.kmeans");
		if(fs.exists(kmeans_datapath)) {
			RemoteIterator<FileStatus> lfs = fs.listStatusIterator(kmeans_datapath);
			while(lfs.hasNext()) {
		...
		if(!fs.exists(new Path(conf.get(Setup.JOB_PATH)+"/data.centroids"+kluster))) {
		    ...
		    res = job_cinnit.waitForCompletion(true)?0:1;
		    if(res==1) return res;
			Resources.appendFile(conf, "Centroid-"+kluster+" Innit: "+Resources.timeParse(LocalDateTime.now().toEpochSecond(ZoneOffset.ofHours(-3))-timenow), "Time.meta", conf.get(Setup.JOB_PATH));
		}
		...
		while(rounds <= conf.getInt(Setup.MAX_ROUNDS, 15)) {
			...
			if(rounds>1) {
				String rs = "0".repeat(2-String.valueOf(rounds-1).length()).concat(String.valueOf(rounds-1));
...
//		    job_k.setMapperClass(KMapper.class);
			RemoteIterator<FileStatus> lst_norm = fs.listStatusIterator(new Path(conf.get(Setup.JOB_PATH)+"/data.norm"));
...
//		    FileInputFormat.addInputPath(job_k, new Path(conf.get(Setup.JOB_PATH)+"/data.norm"));
			...
			for(int k=1;k<=kluster;k++) {
				...
				MultipleOutputs.addNamedOutput(job_k, "GroupList"+k_index, TextOutputFormat.class, Text.class, Text.class);
			}
			...
			if(res==1) return res;
			Resources.appendFile(conf,  "K-means_"+kmeans_suffix+"-round_"+round_suffix+": "+Resources.timeParse(LocalDateTime.now().toEpochSecond(ZoneOffset.ofHours(-3))-timenow), "Time.meta", conf.get(Setup.JOB_PATH));
...
//			job_comp.setMapperClass(KMapper.class);
			RemoteIterator<FileStatus> lst_centroid = fs.listStatusIterator(new Path(centroid_path));
			while(lst_centroid.hasNext()) {
				Path path = lst_centroid.next().getPath();
				if(path.getName().matches("(Centroids-r-)(\\d+)"))
					MultipleInputs.addInputPath(job_comp, path, TextInputFormat.class, KCentroidComparatorMapper.class);
			}
			RemoteIterator<FileStatus> lst_newcentroid = fs.listStatusIterator(new Path(conf.get(Setup.JOB_PATH_KMEANS_ROUND)));
			while(lst_newcentroid.hasNext()) {
				Path path = lst_newcentroid.next().getPath();
				if(path.getName().matches("(Centroids-r-)(\\d+)"))
					MultipleInputs.addInputPath(job_comp, path, TextInputFormat.class, KNewCentroidComparatorMapper.class);
			}
			...
			job_comp.setOutputValueClass(Text.class);
			Path compare_path = new Path(conf.get(Setup.JOB_PATH_KMEANS_ROUND)+"/compare");
			...
			MultipleOutputs.addNamedOutput(job_comp, "Centroids", TextOutputFormat.class, Text.class, Text.class);
			MultipleOutputs.addNamedOutput(job_comp, "Compare", TextOutputFormat.class, Text.class, Text.class);
			res = job_comp.waitForCompletion(true)?0:1;
			if(res==1) return res;
			*/
...
//									break;
					...
					System.out.println("Score: "+compare_complete_flag+" - "+kluster);
