...
//			String ip = st.nextToken();
			...
			int i = 0;
			String newline = "";
			RemoteIterator<FileStatus> lfs = FileSystem.get(conf).listStatusIterator(new Path(conf.get(Setup.JOB_PATH)+"/data.meta"));
			while(lfs.hasNext()) {
...
//			        			qtd_ = Resources.decScale(qtd_,8);
			        			for(int d=cur_div;d<div;d++) {
			        				...
			        				System.out.println("Bin at.:"+cur_bin+"| Tam. Bin:"+bin_size+"| Total:"+total+"| Parte:"+part+"| Qtd ac.:"+qtd+"| Div. at.:"+cur_div+"| Div ac.:"+div+"| Div. it.:"+d);
			        			}
			        			cur_div = div;
			        		}
			        		else System.out.println("Bin at.:"+cur_bin+"| Tam. Bin:"+bin_size+"| Total:"+total+"| Parte:"+part+"| Qtd ac.:"+qtd+"| Div. at.:"+cur_div);
...
//			for(List<Double> centroid : centroids) {}
			...
			int kluster = conf.getInt(Setup.K_CLUSTER_SIZE, 5);
...
//			context.write(new Text("GroupList"+named_index), new Text(ip));
	...
	public static class KReducer extends Reducer<Text,Text,Text,Text> {
		...
		public void reduce(Text key, Iterable<Text> lines, Context context) throws IOException, InterruptedException {
			/*if(key.toString().matches("GroupList(\\d+)")) {
				for(Text t : lines)	mos.write(key.toString(), t, null);
...
//				String[] value = st.nextToken().split(";");
	...
	//					newCentroid.set(i, d + Double.valueOf(v)); //Double.parseDouble(st.nextToken())
						try {
